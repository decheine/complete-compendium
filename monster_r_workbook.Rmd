...
json_data <- fromJSON(paste(readLines(json_file), simplify=TRUE))
```
...
full_filtered_df <- stat_result_df %>% 
  filter(!monster_key %in% base0_monster_keys,
         !monster_key %in% base1_monster_keys) 

stat_result_df
full_filtered_df


# Add monsters with base == 1

split_monsters_df %>% arrange(monster_key)
# base_monsterkeys
combined_monsters <- bind_rows(split_monsters_df, full_filtered_df) 
...
write(toJSON(combined_monsters, pretty = TRUE, auto_unbox = TRUE), "stats_df.json")
